...
const Navbar = () => {
  ...
  return (
    <div className='navbar w-full fixed top-0 left-0 z-50'>
      <div className={` md:flex items-center justify-between py-4 md:px-10 px-7 max-w-screen-2xl m-auto transition-all duration-300 ease-in ${open ? 'bg-white':'bg-transparent'}`}>
      <a href="/" className={` ${open ? 'text-black' : 'text-white'}`}><div className='font-bold text-2xl cursor-pointer flex items-center font-[Poppins]'>
      ...
      <div onClick={()=>setOpen(!open)} className={`text-3xl absolute right-8 top-6 cursor-pointer md:hidden  ${open ? 'text-black': 'text:white'}`}>
      ...
      <ul className={`text-white links md:flex md:items-center md:pb-0 pb-12 absolute md:static  md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-300 ease-in ${open ? 'top-20 bg-white text-black':'top-[-490px]'}`}>
            ...
            <li className={`md:ml-8 text-xl md:my-0 my-7 flex  gap-4 transition-all duration-300 ease-in ${open ? 'flex-col':'flex-row'}`}>
